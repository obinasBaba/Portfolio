...
export default class MagnetElement {
  ...
  startObserving(rootMargin, threshold: number, index: number) {
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        if (entry.isIntersecting) this.start(index);
  ...
  start(index) {
    this.stop(index);
    this.initEvents(index);
    this.elements[0].state.reqAnimationId = requestAnimationFrame(() =>
      this.render(index)
    );
  }
  ...
  render(index) {
    ...
    if (
      ...
      (this.elements[index].state.hover &&
        distanceMouseButton < this.elements[index].distanceToStop)
    ) {
      x =
        (this.eventUtil.mousePos.x -
          (this.elements[index].rect.left +
            this.elements[index].rect.width / 2)) *
        this.elements[index].amounts.distance;
      y =
        (this.eventUtil.mousePos.y -
          (this.elements[index].rect.top +
            this.elements[index].rect.height / 2)) *
        this.elements[index].amounts.distance;
    }
    ...
    this.elements[
      index
    ].dom.style.transform = `translate3d(${this.elements[index].renderedStyles.x.previous}px,
             ${this.elements[index].renderedStyles.y.previous}px,
              0)`;

    this.elements[index].state.reqAnimationId = requestAnimationFrame(() =>
      this.render(index)
    );
  }
}
